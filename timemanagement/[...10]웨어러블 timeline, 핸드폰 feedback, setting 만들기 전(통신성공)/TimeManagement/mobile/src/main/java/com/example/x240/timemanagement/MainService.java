...
public class MainService extends WearableListenerService
        ...
        for (DataEvent event : dataEvents) {
            ...
            if (event.getType() == DataEvent.TYPE_CHANGED) {
                ...
                if (path.equals("/ADDNEWDATA_PATH")) {      //etc data
                    ...
                    save_values(date, tagname, actStartTime, actEndTime);   //data가 추가되면 현재 checkBox를 2로 해놓았기 때문에(원래는 10으로 할거) wear, phone todo리스트에도 떠야하는데 뜨지도 않고... database dataCount도 안바뀌고..
                    //확인 결과 이 함수(onDataChanged)가 아예 호출이 안됨
                }
                    ...
                    int todocheck = dataMapItem.getDataMap().getInt("TODOCHECK");
                    String actstarttime = dataMapItem.getDataMap().getString("ACTSTARTTIME");
                    String actendtime = dataMapItem.getDataMap().getString("ACTENDTIME");
    ...
    private void save_values(String date, String tagname, String actStartTime, String actEndTime) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
