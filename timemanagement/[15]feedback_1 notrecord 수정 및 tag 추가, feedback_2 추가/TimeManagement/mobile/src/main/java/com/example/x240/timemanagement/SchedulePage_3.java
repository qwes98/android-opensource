    ...
    private void make_timeline() {
        ...
        if(cursor.moveToFirst()) {
            while (cursor.isAfterLast() == false) {
                /*
                if (cursor.getInt(3) == 0) {      //하지 않은 일 cursor.moveToNext();
                    continue;
                }
                */
                if (cursor.getInt(3) == 1 || cursor.getInt(3) == 2 || cursor.getInt(3) == 4 || cursor.getInt(3) == 5) {     //하지 않은 일 제외
                    ...
                    if (todoName.equals("Sleeping")) {   //sleeping 처리는 따로 해주어야 (오전 5시 이후만 색칠)
                    ...
                    if (startTimeNum != endTimeNum) {
                        if (endTimeNum <= 400) {             //endTime이 날을 넘긴경우
                            if (startTimeNum <= 400) {
                                ...
                                inputTodoName(todoName, startTimeNum);          //avgTime 찾는 알고리즘 찾기 전까지는 그냥 시작시간에 표시하는걸로..
                            } else {
                                //두번 색깔 칠하기
